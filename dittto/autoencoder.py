...
def generate_autoencoder_models(input_shape:int, **kwargs) -> tuple:
    '''
    ...
    Sample Usage:
        autoencoder, encoder, decoder = generate_autoencoder(input_shape=10, encoder_dense_layers=[20, 18], bottle_neck=16, decoder_dense_layers=[18, 20], decoder_activation='sigmoid')
        autoencoder, encoder, decoder = generate_autoencoder(input_shape=128)        
...
def generate_synthetic_data(model_name: str, original_df: pd.DataFrame, minority_class_column: str = 'class', 
    ...
    input_shape = original_df.shape[1]
    if input_shape < 0:
        raise ValueError("Input shape must be greater than 0.")
    ...
    Args:
    - model_name (str): Name of the autoencoder model to use. Possible values are 'single_encoder', 'balanced', and 'heavy_decoder'.
    - original_df (pandas.DataFrame): Original dataset.
    - minority_class_column (str): Name of the column containing the minority class label. Default is 'class'.
    - minority_class_label (str): Label of the minority class. Default is '0'.
    - decoder_activation (str): Activation function for the decoder layers. Default is 'sigmoid'.
    - epochs (int): Number of epochs to train the autoencoder. Default is 100.

    Returns:
    - synthetic_df (pandas.DataFrame): Synthetic dataset.

    Sample Usage:
    synthetic_df = generate_synthetic_data(model_name='single_encoder', original_df, minority_class_column='class', minority_class_label='0', decoder_activation='softmax', epochs=100)
    synthetic_df = generate_synthetic_data(model_name='balanced', original_df, minority_class_column='class_name', minority_class_label='0', decoder_activation='sigmoid', epochs=100)
    """
